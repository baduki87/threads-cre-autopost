...
| 21:13 | `daily-post.yml` | 메모 or **AI×부동산** | 승인된 것 우선, 없으면 자동 발행 |
...
| 21시 | 메모 있으면 임장기, 없으면 **AI×부동산** | 조회층 관심사 2위가 AI(64) 인데 경쟁이 비어 있다 |

### 2026-09-18: 21시를 뉴스에서 AI 로 바꿨다

뉴스 10건의 조회 평균이 493 으로 가장 높았지만 **좋아요 합계 8, 남이 단 댓글 1**
이었다. 그날 기사가 세면 조회가 나오는 것뿐이고 우리 실력이 아니다.
목표가 팔로워인데 뉴스는 팔로우를 만들지 못했다(10일에 +4).

AI × 부동산을 택한 근거는 셋이다.
1. 조회층 관심사가 부동산 74 / **AI 64** 다
2. 부동산 계정 중 이걸 제대로 하는 곳이 없다
3. 계정 주인이 실제로 하는 일이다 — 중개용 CRM 자체 제작,
   2016년 이세돌·알파고 4국 심판, 클로드로 홈페이지 제작

무엇보다 **판단이 아니라 방법**이라 자동화가 할 수 있는 몇 안 되는 영역이다.

**성과 데이터가 아니라 가설이다.** 2~3주 뒤 조회 총량이 크게 떨어지면 되돌린다.

21시는 승인을 거치지만, **승인이 없어도 뉴스 글은 자동으로 나간다.**
승인 대기로 6건 연속 이 슬롯이 비었고 그 시간대가 반응이 가장 좋은 자리였다.

**임장기는 예외다.** 회원님 메모에서 나온 판단이 들어 있어 승인 없이 내보내지
않는다. 이 구분이 "의견은 메모에서만" 규칙의 마지막 관문이다.
