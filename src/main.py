...
#   17시  질문      — 댓글이 가장 많이 달린다
#   21시  메모 있으면 임장기, 없으면 AI × 부동산
#
# 2026-09-18 에 21시를 뉴스에서 AI 로 바꿨다. 뉴스 10건의 조회 평균은 493 으로
# 가장 높았지만 좋아요 합계가 8, 남이 단 댓글이 1 이었다. 그날 기사가 세면
# 조회가 나오는 것뿐이고 우리 실력이 아니다. 목표가 팔로워인데 뉴스는
# 팔로우를 만들지 못했다(10일에 +4).
#
# 대신 조회층 관심사 2위가 AI(64) 이고, 부동산 계정 중 이걸 하는 곳이 없다.
# 계정 주인이 실제로 하는 일이라(중개용 CRM 자체 제작, 이세돌·알파고 4국 심판)
# 가짜가 아니다. 무엇보다 **판단이 아니라 방법**이라 자동화가 할 수 있다.
#
# 근거가 성과 데이터가 아니라 가설이다. 2~3주 뒤 조회 총량이 크게 떨어지면
# 뉴스로 되돌린다.
SLOT_KIND = {"08": "방법론", "17": "질문", "21": "AI"}

# 카드를 붙이지 않는 유형. 카드로 만들 만한 수치가 없고, 실측으로도
# AI 카드(평균 291)가 글만 올린 것(535)보다 못했다.
NO_CARD_KINDS = {"질문", "방법론", "AI활용"}
...
def _build_post(st: dict, *, allow_memo: bool, slot: str = ""):
    ...
    if want == "AI":
        # AI 주제는 연재로 쪼개지 않는다. 한 편에 방법이 다 들어가야 쓸모가 있다.
        pick, kind = pick_fallback(st=st, category="AI"), "AI활용"
    elif want == "방법론":
